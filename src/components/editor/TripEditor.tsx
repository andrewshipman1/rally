...
export function TripEditor({
  ...
  trip: Trip & { theme: Theme | null; blocks: Block[] };
  themes: Theme[];
}) {
  ...
  const [deadline, setDeadline] = useState(trip.commit_deadline ? trip.commit_deadline.split('T')[0] : '');
  const [selectedTheme, setSelectedTheme] = useState<string | null>(trip.theme_id);
  const [phase, setPhase] = useState(trip.phase);
  const [blocks, setBlocks] = useState<Block[]>(trip.blocks || []);
    ...
    <div style={{ maxWidth: 500, margin: '0 auto', padding: '24px 20px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button
            onClick={() => router.push('/')}
            style={{ background: 'none', border: '1px solid rgba(0,0,0,0.1)', borderRadius: 10, padding: '8px 12px', cursor: 'pointer', fontSize: 13, color: '#888' }}
          >
            ← Back
          </button>
          <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: 22, fontWeight: 800, color: '#1a3a4a', margin: 0 }}>
            Edit trip
          </h1>
        </div>
      ...
      {/* Trip details */}
      <div style={{ background: '#fff', borderRadius: 16, padding: 20, boxShadow: '0 2px 12px rgba(0,0,0,0.06)', border: '1px solid rgba(0,0,0,0.04)' }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: '#1a3a4a', marginBottom: 4 }}>Trip details</div>
      ...
      <div style={{ marginTop: 20 }}>
        <BlockEditor tripId={trip.id} blocks={blocks} onBlocksChange={setBlocks} />
      </div>
    </div>
  );
}
