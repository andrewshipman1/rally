...
<body style="margin:0; padding:0; background:${bg}; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; color:${ink};">
  <!-- Outer-table bg is a fixed neutral page color (#ece7dc, mockup ref) so the
       inner card reads as a contained surface against any of the 17 theme bgs. -->
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#ece7dc; padding:40px 20px;">
    <tr>
      <td align="center">
        <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="width:560px; max-width:560px; table-layout:fixed; background:${bg}; border-radius:18px; overflow:hidden; box-shadow:0 8px 32px rgba(0,0,0,0.08);">
          <tr>
            <td width="560" style="padding:0;">
              <div style="width:100%; max-width:560px; box-sizing:border-box; background:${surface}; color:${stickerBg}; overflow:hidden; white-space:nowrap; padding:6px 0; border-bottom:2px solid ${surface};">
          ...
          <tr>
            <td width="560" style="background:${accent}; padding:28px 22px 24px; color:${onSurface}; position:relative;">
          ...
          <tr>
            <td width="560" style="padding:24px 22px 8px; background:${bg}; color:${ink};">
          ...
          <tr>
            <td width="560" align="center" style="text-align:center; padding:8px 22px 4px; background:${bg};">
          ...
          <tr>
            <td width="560" style="text-align:center; font-size:11px; color:${accent2}; padding:18px 22px 24px; background:${bg}; line-height:1.5;">
              or paste this link:<br>
              <a href="${escapeAttr(shareUrl)}" style="color:${accent2}; word-break:break-all;">${escapeHtml(shareUrl)}</a>
            </td>
          </tr>
          <tr>
            <td width="560" style="background:${surface}; color:${onSurface}; padding:16px 22px; text-align:center; font-family:Georgia,'Times New Roman',serif; font-size:13px; letter-spacing:0.3px;">
